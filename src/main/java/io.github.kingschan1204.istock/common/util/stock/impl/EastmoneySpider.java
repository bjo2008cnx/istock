...
public class EastmoneySpider extends DefaultSpiderImpl {
  ...
  /**
   * ReportingPeriod:报告期 ResultsbyDate：披露时间 SGBL：送股比例 ZGBL：转股比例 GXL：股息率 TotalEquity：总股本 AllocationPlan：分配预案 ProjectProgress:方案进度 GQDJR：股权登记日
   * CQCXR：除息除权日
   */
  @Override
  public JSONArray getHistoryDividendRate(String code) throws Exception {
    ...
    String apiUrl = String.format(
        "http://dcfm.eastmoney.com/EM_MutiSvcExpandInterface/api/js/get?type=DCSOBS&token=%s&p=1&ps=50&sr=-1&st=ReportingPeriod&filter=&cmd=%s",
        token, code);
